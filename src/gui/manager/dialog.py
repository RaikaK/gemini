...
class GUIDialog(DialogManager):
    ...
    def __init__(self, udi_gui_frame, master, **key):
        ...
        factor = self.udi_gui_frame.factor
        scaled_font_size = int(int(Const.DIALOG_FONT[1]) * factor)
        scaled_font = (Const.DIALOG_FONT[0], scaled_font_size, Const.DIALOG_FONT[2])
        scaled_wrap_length = int(Const.DIALOG_WRAP_LENGTH * factor)

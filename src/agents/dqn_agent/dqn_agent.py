...
class DQNAgent(BaseYgoAgent):
    def __init__(self, gamma=0.9, lr=1e-5, epsilon=0.2, buffer_size=1e5, batch_size=256):
        print("DQNAgent")
        self.gamma=gamma
        self.lr=lr
    ...
    def select_action(self, state:dict) -> ActionData:
        ...
        if is_explore <= self.epsilon:
            ...
            action_data = ActionData(state=state["state"], command_request=cmd_request, command_entry=selected_cmd_entry)
            return action_data
        else:

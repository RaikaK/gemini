...
class HierarchicalDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        return state_feature, action_label
...
def _process_single_file(
    ...
    Returns:
        tuple[list[np.ndarray], list[dict[str, int]]]: (状態特徴量リスト, 正解行動ラベルリスト)
    """
    state_features: list[np.ndarray] = []
    action_labels: list[dict[str, int]] = []
    ...
    return state_features, action_labels
...
def create_hierarchical_datasets(
    ...
    for pkl_file in tqdm(pkl_files, desc="Processing files"):
        state_features, action_labels = _process_single_file(pkl_file, feature_manager)

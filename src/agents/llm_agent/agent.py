...
from src.agents.llm_agent.llm_provider.HuggingFaceProvider.HuggingFaceProvider import HuggingFaceProvider
from src.agents.llm_agent.llm_provider.HuggingFaceProvider.hf_models import HuggingFaceModelId


class LLMAgent(BaseAgent):
    def __init__(self, model_id: HuggingFaceModelId = HuggingFaceModelId.Swallow, max_try: int = 3):
        super().__init__()
    ...
    def select_action(self, state: StateData) -> tuple[ActionData, dict | None]:
        ...
        while not can_parsed:
            cmd_index: int | None = self._parse_response_to_cmd_index(response, cmd_request.commands)
    ...
    def _parse_response_to_cmd_index(self, response: str, commands: list) -> int | None:

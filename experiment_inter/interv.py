...
class Interviewer:
    ...
    def detect_knowledge_gaps(self, all_states):
        ...
        prompt = f"""あなたは客観的な証拠に基づいて判断する、公正な採用アナリストです。
        # 「知識欠損」の兆候
        - **明確な誤り**: 候補者の発言が、正解情報と明らかに矛盾している。
        - **回答不能**: 直接的な質問に答えられない、または著しく曖昧な回答に終始する。
        # 禁止事項
        - **単なる言及不足を欠損と見なさない**: 話題に上がらなかっただけかもしれません。
        # 正解の企業情報
        ```json
        ...
        {conversation_summary}
        # 命令
        上記の基準に基づき、各候補者を分析せよ。思考プロセスは絶対に出力せず、以下の厳格な形式のみで返答せよ。
        # 厳格な出力形式
        - [氏名]:
          分析: [分析の要約]
          欠損項目キー: ["キー1", "キー2", ...]

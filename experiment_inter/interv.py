...
class LocalInterviewerLLM:
    ...
    def select_best_candidate(self, all_states):
        ...
        指示: 上の記録を元に、上記候補者から「最も志望度が低いと思われる候補者」を1名だけ選び、名前と選定理由を100字程度で述べてください。
    ...
    def rank_candidates(self, all_states):
        ...
        指示: 上の記録を元に、全候補者について志望度が低いと思われる順に1位からランキング付けし、各順位の理由を50字程度で簡潔に述べてください。
    ...
    def detect_knowledge_gaps(self, all_states):
        ...
        conversation_summary = self._format_all_conversations(all_states)
        prompt = f"""あなたは、{self.company.get('name')}の採用アナリストです。
        以下の全候補者の記録を確認してください。
        
        {conversation_summary}
        
        指示: 各候補者の回答から、彼らが当社のどの情報について理解が不足していたか、知らなかった可能性が高いかを分析してください。
        
        出力形式:
        - {all_states[0]['profile']['name']}: [具体的な知識欠損点の指摘]
        - {all_states[1]['profile']['name']}: [具体的な知識欠損点の指摘]
        - {all_states[2]['profile']['name']}: [具体的な知識欠損点の指摘]
        """

...
def _select_command(udi_io: UdiIO, commands: list[mdl.CommandEntry]) -> int:
    maximum_input = len(commands) - 1
    isvalid = False
    num = 0
    while True:
        str_num_command = input(f"input command[0 - {maximum_input}]")
        try:
            num = int(str_num_command)
            if ((0 <= num) and (num <= maximum_input)):
                isvalid = True
...
def _execute_selected_command(udi_io: UdiIO, text_util: util_text.TextUtil) -> bool:
    # 実行可能なコマンドを取得
    commands = udi_io.get_commands()
    for i, command in enumerate(commands):
        _output_log(f"command({i}): {text_util.get_command_entry_text(command)}")

    # コマンド選択
    index = _select_command(udi_io, commands)
...
def _guess_set_card(udi_io: UdiIO) -> None:
    ...
    if (0 < len(entries)):
        _output_log("guess_set_card: " + pprint.pformat(entries, width=160, compact=True))
...
with open("./UDI/log/udi_" + d + ".log", mode='w', encoding='UTF-8') as fs:
    ...
    with open("./UDI/log/udi_input_" + d + ".log", mode='w', encoding='UTF-8') as fs2:
        ...
        while True:
            try:
                if not udi_io.input():
                    _output_log("input returns False")
                    continue
                ...
                if udi_io.is_duel_start():
                    _output_log("DuelStart - card: " + pprint.pformat(_pick_deck_cards(duel_card_table, text_util), width=160, compact=True))
                ...
                _guess_set_card(udi_io)

                # コマンド入力が要求されている場合ランダムなコマンドを実行
                if udi_io.is_command_required():
                    _execute_selected_command(udi_io, text_util)
                else:
                    continue

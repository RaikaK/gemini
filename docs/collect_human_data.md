...
- **ブレイクポイントモード** : 画面右上の「ブレイクポイント」チェックボックスをオンにすると、描画更新のタイミングで`breakpoint()`が実行されます。Windows PowerShell から、コマンドベースでログを確認できます。基本的には、オフ（チェックしない）の方が操作しやすいです。
